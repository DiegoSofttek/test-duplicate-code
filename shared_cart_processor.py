...
def procesar_orden_compra(carrito, cliente, configuracion_tienda):
    """Procesa un carrito de compras y genera el resumen de orden.

    Args:
        carrito: Iterable de diccionarios con datos de cada producto.
        cliente: Diccionario con datos del cliente (id, email, etc.).
        configuracion_tienda: Configuración de la tienda (IVA, envío, etc.).

    Returns:
        Diccionario con metadata, resumen financiero, logística y detalle de artículos.
    """
    total_sin_iva = 0
    total_descuentos = 0
    peso_total = 0
